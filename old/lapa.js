if(!window.LaPa) {
    window.LaPa = {
        ...
        'CONF': {'runScriptsAsFuncContainer':true,'legacyPages':true},
        'STACK': {},
        ...
        'fixJSforDB':function(){
            ...
            exportJS={};
            for(i in jsList){
                exportJS[jsList[i]]={'body':LaPa.JS[jsList[i]].body};
            }
            return exportJS;
        },
        ...
        'io': function (callback, url, params, key, meta, queue) {
            key = key ? key : url;
            if (LaPa.STACK[key] && !LaPa.STACK[key].queue && LaPa.STACK[key].delay==false){LaPa.log(key+' duplicated call, already loading...','STACK');return;}
            if (!LaPa.STACK[key]) {
                if (!callback || !url){LaPa.log('Callback or Url not presented, abort...','STACK');return;}
                LaPa.STACK[key] = {
                ...
                if (queue && Object.keys(LaPa.STACK).length > 0) { // TODO : Check for all exist items are in queue
                    ...
                    for (i in LaPa.STACK)if (!LaPa.STACK[i].queue){LaPa.log('Thread used by '+i,'STACK');return;};
        ...
        'loadScript': function (source, meta) {
            ...
            LaPa.log(meta.url + ' Loaded', 'SCRIPT');
            if (!LaPa.JS[meta.url]) LaPa.JS[meta.url] = {};
            LaPa.JS[meta.url].body = source;
            LaPa.JS[meta.url].loaded = LaPa.JS[meta.url].loaded ? LaPa.JS[meta.url].loaded : false;
            LaPa.saveDB('JS');
            if (LaPa.JS[meta.url].preparingRun) LaPa.runScript(meta.url);
        },
        ...
        'makeIndicator': function (state) {
            if (LaPa.indicator) {
                ...
                LaPa.indicator.onclick = function(){if(confirm('Ускорено технологией LaPa Software, нажмите ОК чтобы узнать как ускорить ваш сайт.')){localStorage.clear();location.href='http://lapa.96.lt';}};
                LaPa.indicator.innerHTML = '<div id="indicator_loader" style="visibility: hidden;">' +
                    '<svg version="1.1" id="loader-1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="40px" height="40px" viewBox="0 0 50 50" style="enable-background:new 0 0 50 50;" xml:space="preserve"><path fill="#fff" d="M25.251,6.461c-10.318,0-18.683,8.365-18.683,18.683h4.068c0-8.071,6.543-14.615,14.615-14.615V6.461z"><animateTransform attributeType="xml" attributeName="transform" type="rotate" from="0 25 25" to="360 25 25" dur="0.6s" repeatCount="indefinite"/></path></svg>' +
        ...
        'indicatorState': function (state) {
            if(!document.getElementById('indicator')){LaPa.makeIndicator(state);return}
        ...
        'check':function (response) {
            ...
            if(LaPa.CONF['domain']) {
                ...
                LaPa.io(LaPa.check, 'http://lapa.96.lt/domain.php', {'url': location.host,'ver':JSON.stringify(LaPa.version)}, 'check');
            }
            return true;
        },
        'parseHTML': {
            'modifyLinks': function(source,dom){
                if(dom){container=dom;}else{
    ...
    window.native_addEventListener('LaPaLoad',function(){LaPa.log('LaPa called event LaPaLoad','Navigator')});
    window.native_addEventListener('LaPaRender',function(){LaPa.log('LaPa called event LaPaRender','Navigator')});
